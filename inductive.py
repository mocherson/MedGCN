...
for epoch in range(epochs):
    ...
    medgcn.train()
    optimizer.zero_grad()
    adj_recon, z = medgcn(train_fea_mats, train_adj_mats, train_adj_masks )
    train_loss=lossfun(adj_recon, train_adj_mats, train_adj_masks, adj_losstype)
    train_loss.backward()
    optimizer.step()
    ...
    medgcn.eval()
    with torch.no_grad():
        adj_recon, z = medgcn(fea_mats, adj_mats, masks )
